<template>
    <section class="bg-primary-color h-[100vh] text-white w-full">
        <div class="bg-quartinary-color h-[50vh] w-[40vw] m-auto translate-y-[50%] p-4">
            ...
            <div class="pt-4">
                <button class="px-16 w-1/2 block mx-auto py-2 bg-tertiary-color rounded-sm hover:bg-primary-color" @click="register">Sign up</button> 
                <router-link to="/" custom v-slot="{ navigate }">
                    <span @click="navigate" @keypress.enter="navigate" role="link" class="cursor-pointer px-8 py-2 w-[300px] block mx-auto hover:text-secondary-color">Already an account? Login instead</span>
                </router-link>
            </div>
        </div> 
    </section>
