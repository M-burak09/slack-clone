<template>
    <section class="bg-tertiary-color text-white h-12 select-none flex justify-center w-full">
        ...
        <input type="text" v-model="input" :placeholder="'Search ' + this.$store.state.organization" class="rounded-md border border-quartinary-color bg-primary-color ml-4 px-2 my-2 w-1/2 max-w-[600px]"/>
        ...
        <div class="absolute right-4 top-2 flex">
            ...
            <img @click="profileModal = true" class="cursor-pointer w-6 h-6 mt-1 rounded-md" :src="this.$store.state.userpic" alt="">
        ...
        <div v-show="profileModal" class="text-secondary-color absolute z-30 right-4 top-10 bg-[#222529] rounded-lg border border-quartinary-color py-4 w-80">
            <div class="flex px-6 mb-2">
                <img class="w-10 h-10 my-auto rounded-lg" :src="this.$store.state.userpic" alt="">
                <div class="ml-3">
                    <p class="font-bold text-white">{{this.$store.state.username}}</p>
                    <div class="flex">
            ...
            <ul>
                ...
                <span class="border-b border-quartinary-color w-full inline-block"></span>
                <li class="dialog-items mt-2" @click="handleSignOut" v-if={isLoggendIn}> Sign out of {{this.$store.state.organization}}</li>
            </ul>
